...
func taskPluginProtocolHandlers(protocol, operation string) ([]gin.HandlerFunc, error) {
	...
	case "openai_responses.create":
		return []gin.HandlerFunc{
			middleware.RouteTag("relay"), middleware.AnomalyObservation(), middleware.SystemPerformanceCheck(), middleware.TokenAuth(),
			middleware.StreamingOnly(), middleware.ModelRequestRateLimit(), middleware.PinTaskPluginEndpoint(), middleware.PrepareTaskPluginEndpoint(), middleware.Distribute(),
			func(c *gin.Context) {
	...
	case "openai_video.create":
		return []gin.HandlerFunc{
			middleware.RouteTag("relay"), middleware.AnomalyObservation(), middleware.TokenAuth(), middleware.SystemPerformanceCheck(),
