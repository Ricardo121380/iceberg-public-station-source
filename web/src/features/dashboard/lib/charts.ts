...
export function getDashboardChartColors(domainLength: number): string[] {
  ...
  const holiday = getOpeningHoliday().holiday
  if ((preset === 'iceberg' || holiday) && domainLength > 0) {
    const resolvedTheme = document.documentElement.classList.contains('dark')
      ? ('dark' as const)
      : ('light' as const)
    return icebergSeriesRamp(resolvedTheme, domainLength, holiday)
  }
...
export function processUserChartData(
  ...
  const userColors = getOpeningHoliday().holiday
    ? getDashboardChartColors(USER_COLORS.length)
    : USER_COLORS
  const tt: TFunction = t ?? ((x) => x)
  const { config } = getCurrencyDisplay()
  const quotaPerUnit = config.quotaPerUnit
